...
static const builtin_cmd_t builtins[] = {
    ...
    {"help", &help}, {"?", &last_status}, {"prompt", &set_prompt},
    {NULL, NULL}};

int is_builtin(char *command) {
  for (size_t index = 0; builtins[index].name; index++) {
    if (!strcmp(command, builtins[index].name))
      return 1;
  }
  return 0;
}
...
void cd(char **tokens) {
  ...
  if (!tokens[1]) {
    ...
    snprintf(pathbuf, PATH_MAX, "%s", home);
    ...
    snprintf(pathbuf, PATH_MAX, "%s/%s", home, tokens[1] + 1);
  } else {
    snprintf(pathbuf, PATH_MAX, "%s", tokens[1]);
  }
...
void set_prompt(char **tokens) {
  ...
  if (context.prompt != default_prompt)
    free(context.prompt);

  if (strcmp(tokens[1], "") == 0) {
    context.prompt = default_prompt;
  } else
    context.prompt = strdup(tokens[1]);
}
...
char **parse_input(char *input) {
  ...
  tokens_size = TK_ARR_INIT_SIZE;
  tmp_size = TK_MAX_SIZE;
  ...
  for (token_index = input_index = 0; (cur_char = input[input_index]) != '\0';
    ...
    if (!isblank(cur_char)) {
      tmp_index = 0;
      while (!isblank(cur_char = input[input_index]) && cur_char != '\0') {
